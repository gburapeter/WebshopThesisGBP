import { Inertia } from "@inertiajs/inertia";
import React from "react";

const NotificationListItem = ({ notification }) => {
    return (
        <li className="border-b-2 border-indigo-100">
            <div className="py-5 px-4 flex justify-between border-l-4 border-transparent hover:bg-indigo-200 bg-transparent">
                <div className="sm:pl-4 pr-8 flex sm:items-center">
                    {/* Icon  */}
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className="w-20 h-20 pr-3"
                    >
                        <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z"
                        />
                    </svg>

                    <div className="space-y-1">
                        {/* Details  */}
                        <p className="text-sm text-gray-700 font-bold tracking-wide">
                            {notification.data.details}
                        </p>
                        {/* ref number  */}
                        <p className="text-sm text-indigo-500 font-medium">
                            #{notification.data.reference_number}
                        </p>
                    </div>
                </div>

                <div className="pr-4 flex flex-col justify-between items-end">
                    {/* View button  */}
                    <div>
                        <div className="relative">
                            <a
                                href={route(notification.data.route, [
                                    notification.data.id,
                                ])}
                                className="text-sm text-gray-500 font-semibold hover:underline hover:text-gray-700"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    strokeWidth={1.5}
                                    stroke="currentColor"
                                    className="w-6 h-6"
                                >
                                    <path
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"
                                    />
                                    <path
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"
                                    />
                                </svg>
                            </a>
                        </div>
                    </div>

                    {/* Delete button  */}
                    <a
                        href="#"
                        onClick={() =>
                            Inertia.delete(
                                route("notifications.delete", [notification.id])
                            )
                        }
                        className="text-sm text-gray-500 font-semibold hover:underline hover:text-gray-700"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            strokeWidth={1.5}
                            stroke="currentColor"
                            className="w-6 h-6"
                        >
                            <path
                                strokeLinecap="round"
                                strokeLinejoin="round"
                                d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                            />
                        </svg>
                    </a>
                </div>
            </div>
        </li>
    );
};

export default NotificationListItem;
